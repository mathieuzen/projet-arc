...
var xss = require('lib/richtext').xssFilter({
    video: true,
    image: true,
    link: true
});
...
TopicSchema.options.toObject.transform =
    TopicSchema.options.toJSON.transform = function(doc, ret) {
        ...
        if (ret.upvotes) ret.upvotes = ret.upvotes.map(function(v) {
            return v.author;
        });
        if (ret.downvotes) ret.downvotes = ret.downvotes.map(function(v) {
            return v.author;
        });
        if (ret.abstentions) ret.abstentions = ret.abstentions.map(function(v) {
            return v.author;
        });
        if (ret.clauses) ret.clauses = ret.clauses.sort(function(a, b) {
