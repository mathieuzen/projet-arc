...
exports.all = function all(params, fn) {
    ...
    Topic
        .find(query)
        .select('id topicId mediaTitle tag participants votes createdAt updatedAt closingAt publishedAt deletedAt status open closed links author authorUrl forum verified')
...
exports.update = function update(id, data, fn) {
    ...
    function onget(err, topic) {
        ...
        var submittedIds = clauses.map(function(c) {
            return c.id
        });
        var persistedIds = topic.clauses.map(function(c) {
            return c.id
        });
        var clausesToDelete = persistedIds.filter(function(i) {
            return !~submittedIds.indexOf(i)
        });
        log('submittedIds: %j', submittedIds);
        log('persistedIds: %j', persistedIds);
        log('clausesToDelete: %j', clausesToDelete);
        clausesToDelete.forEach(function(id) {
            topic.clauses.pull({
                _id: id
            })
        });
...
exports.comments = function comments(id, paging, fn) {
    ...
    var query = {
        $or: [{
            ...
            context: 'topic',
            reference: mongoose.Types.ObjectId(id)
        }]
    };

    if (paging.exclude_user) {
        query.author = {
            $ne: paging.exclude_user
        };
    }
